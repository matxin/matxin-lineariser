		...
		for ord in range(0, len(k)): #{
			...
			if j == lin_h: #{
				...
				if childs == '': #{
					...
					childs = childs + '|' + str(ord) + '/' + rule[j][0] + '/' + rule[j][1];
					config_childs = config_childs + '|' + rule[j][0] + '/' + rule[j][1];
				#}
			#}
...
for c in configs: #{
	...
	for r in configs[c]: #{
		...
		print('%.2f\t%d\t%d\t%s\t%s' % (prob, total, configs[c][r], c, r), file=sys.stderr)
		head = r.split('!')[0].split('/');
		deps = r.split('!')[1].split('|');

		print('<def-rule p="%.4f">' % (prob));
		print('  <NODE ord="%s" si="%s" pos="%s">' % (head[0], head[2], head[1]));		
		for dep in deps: #{
			deprow = dep.split('/');
			print('     <NODE ord="%s" si="%s" pos="%s"/>' % (deprow[0], deprow[2], deprow[1]));		
		#}
		print('  </NODE>');
		print('</def-rule>');
	#}
