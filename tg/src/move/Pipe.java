...
import is2.transitionS2.GuideOracle;
...
final public class Pipe extends PipeGen {
	...
	throws Exception {
		...
		//	if (num1 % 100 == 0)  
			...
			Parse p = new Parse(is.length(num1));
			for (int k=0;k<p.heads.length;k++) p.heads[k]=(short) is.heads[num1][k]; 

	
			int[] orderp = GuideOracle.inorder(p);
			
			Linear l = new Linear(is,num1,false);
			
			// a method:
			/**
			 * 1. Compute projective blocks
			 * 2. Move projective block to the point where it belongs to:
			 * The head of  left node where the block is adjacent. 
			 * 
			 */
			
			
			for(int k=0;k<is.length(num1);k++) {
