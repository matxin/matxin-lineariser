...
//					if (cnt>13 && hfg.feats[j]!=null) System.out.println(""+hfg.feats[j][0]+" "+(hfg.feats[j][0]==null));
			...
			List<Order> orders = getLeftN(l,root,30,8);
			if (orders.size()>30) orders = orders.subList(0, 30);
...
//			System.out.println("best: "+orders.get(best));
	 ...
	 * @return
	 */
	private static int languageModel(SentenceData09 instance, List<Order> orders, NgramLanguageModel<String> languageModel) {
		...
		for(int o=0;o<orders.size();o++) {
			...
			if ((prob/score)>bestEntroy) {
				bestEntroy=prob/score;
				best=o;
			}
		
			
			System.out.println(""+s+"\t"+prob+" "+best+" "+bestEntroy+"\t"+orders.get(o).isPartGold()+"\t"+orders.get(o).wrong());
