...
final public class Pipe extends PipeGen {
	...
	throws Exception {
		...
		//	if (num1 % 100 == 0)  
			...
			// drop sentences without a dependency annotation 
			if(instance1.heads[1]==-1) continue;
			
			if (is !=null) depReader.insert(is,instance1);
		...
		for(int w1 : nds) {
			for(int w2 : nds) {
				if(w1==w2) continue;
					...
					f.clear();
					extractor[0].extractFeaturesX(is, inst, w1, w2, 1, f);
	...
	public void createVector1(Instances is, int n, int[] nodes, int head,FV f) {
		...
		for(int i=0;i<nodes.length-1;i++){
			...
			for(int j =i+1;j<nodes.length;j++) {
				...
				extractor[0].extractFeaturesX(is, n, nodes[i], nodes[j],1, f);	
			}
