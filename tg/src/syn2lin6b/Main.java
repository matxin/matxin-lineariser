...
public class Main {
	...
	public static void main (String[] args) throws Exception
	{
		...
		if (options.train && options.contfile==null) {
		...
		if (options.contfile !=null) {
			...
			Decoder decoder = readAll(options.contfile, pipe, params);
			
			Long2IntInterface long2int = new Long2Int(params.parameters.length);
			DB.println("li size "+long2int.size());
			...
			params.total = new float[params.parameters.length];
			for(int i=0;i<params.parameters.length;i++) {
				params.total[i] = params.parameters[i]*((options.numIters)*is.size()	);
			}
		...
		if (options.test) {
			...
			Decoder decoder = readAll(options.modelName, pipe, params);
	...
	public  static Decoder readAll(String modelName, Pipe pipe,ParametersFloat params) throws IOException {
...
//					if (n<27838 && px[is.gpos[n][k]].startsWith("V")) continue;
...
//				List<Order> orders = getLeftN(l,root,10,6,params,pipe.extractor[0],is,n,(int)upd); // 8 would be better
				List<Order> orders = getLeftN(l,root,10,6); // 8 would be better
			//	List<Order> orders = getLeftN(l,root,10,8); // 8 would be better
...
//					if (cnt>13 && hfg.feats[j]!=null) System.out.println(""+hfg.feats[j][0]+" "+(hfg.feats[j][0]==null));
			...
			for(int k=0;k<l.wordChilds.length;k++) {
				...
				*/
				...
				if (options.tt ==0)
				for (int j=0;j<l.results[node].length;j++) 
					Arrays.sort(l.results[node][j].path);
...
//			List<Order> orders = getLeftN(l,root,40,10);
			List<Order> orders = getLeftN(l,root,20,8);
...
//			ArrayList<Integer> al = getLeft2(l,root,added,cnt);
			...
			int all[] = new int[l.wordChildsResult.length];
			int pos=0;
			for(int k=0;k<al.size();k++) {
	...
	//			System.exit(0);
			...
			for(int j = 0; j < formsNoRoot.length+1; j++) {
			...
			for(int j = 0; j < formsNoRoot.length; j++) {
				...
				if (jx.get(instance.heads[w])==null) {
					DB.println("error "+j);
					DB.println(""+instance);
					DB.println("al "+al);
					for(int p : all) {
				...
				else heads[j] =jx.get(instance.heads[w]); //inostance.heads[
	...
	//		System.out.println("pred:"+s.toString()); 
		...
		System.out.println("BLEU "+bleu/cnt);
	}
		...
		for(int o=0;o<orders.size();o++) {
			...
			for(int j = 0; j < instance.length()-1; j++) {
				...
				if (s.length()>0) s.append(" ");
					...
					int w= all[j+1];
					if (instance.lemmas[w].startsWith("<root-")) continue;
