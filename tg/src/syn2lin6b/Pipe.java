...
final public class Pipe extends PipeGen {
	...
	throws Exception {
		...
		while (true) {
			...
			if (instance.heads[1]==-1) {
				
				
				// parser might no have parsed all sentences.
			...
			for (int i1 = 0; i1 < fs.length; i1++){	
				w =fs[i1];
				if (w==null) continue;
			//	if (w[i1].startsWith("id=")) continue;
		...
		DB.println("create "+parsed+" instances of "+ic+" read instances");
		is.init(ic, mf);
		...
		//	if (num1 % 100 == 0)  
			...
			for(int k=0;k< instance1.pheads[k];k++) {
				
				if ( instance1.pheads[k]==-1) {
					DB.println("instance1.pheads[k] "+k+" "+instance1.pheads[k]+" num:"+num1);
					System.exit(0);
				}
			}
			
			depReader.insert(is,instance1);
		
			int emptyGLemma = 0;
			// check lemmas
			int inst =is.size()-1;
			for(int k=0;k<is.length(is.size()-1);k++) {
				
				// clear form
				is.forms[inst][k]=-1;
				
				if (is.glemmas[is.size()-1][k]<0) {
					DB.println("lemma "+instance1.lemmas[k]+" "+mf.getValue(WORD,depReader.normalize( instance1.lemmas[k])));
					emptyGLemma++;
				}
			}
			
			if (emptyGLemma>0) DB.println("error lemma in train data is unkown "+emptyGLemma+ "  in sentence "+(is.size()-1)+" size "+is.length(is.size()-1));
	...
	public DataNN fillVector(F2SF f, Instances is, int inst, DataNN d, int nds[], Path[][] paths) {
		...
		for(int w1 : nds) {
			for(int w2 : nds) {
				if(w1==w2) continue;
					...
					f.clear();
					extractor[0].extractFeaturesX(is, inst, w1, w2, 1, f,paths);
...
//					}
	 ...
	 * @param pred2
	 */
	public void createVector1(Instances is, int n, int[] nodes, int head,FV f, Path[][] paths) {
		...
		for(int i=0;i<nodes.length-1;i++){
			...
			for(int j =i+1;j<nodes.length;j++) {
				...
				extractor[0].extractFeaturesX(is, n, nodes[i], nodes[j],1, f, paths);	
			}
