...
public class Decoder {
	 ...
	 * @param pipe
	 */
	public static int[][] decode(Instances[] is, int n, Pipe pipe, ParametersFloat params, float upd, String[] ruleName) {
...
//			DB.println(snt.get(r)+"\t\t "+snt.get(r).head+"  rule:"+rule[rules[r][0]]);
		//	DB.println(r+"\t"+snt.get(r)+"  rule:"+rule[rules[r][0]]);


			if (rule[rules[r][0]].length()>2 && ! rule[rules[r][0]].equals("<None>")) {
			//	DB.println(snt.get(r)+"\t\t "+snt.get(r).head+"  rule:"+rule[rules[r][0]]);

				String rules4Node[] =  rule[rules[r][0]].split(";");
							
				for(String ruleN : rules4Node) {
				
				Word w = new Word();
				String[] rl = ruleN.split(":");
				...
				if (rl[0].equals("r")) {
					...
					//			w.lable =rl[1];
				//				w.plable =rl[1];
								snt.get(r).lemma=rl[1];
								snt.get(r).form=rl[1];
								DB.println("relabel "+rl[1]);
								// add as new node later
				//				newNodes.add(w);				
...
//								snt.get(r).lable=rl[3];
		...
		//		DB.println("in\t"+snt.get(r)+"\t\t "+"  rule "+rule[rules[r][1]]);
				...
				String rules4Node[] =  rule[rules[r][1]].split(";");
				...
				DB.println("apply insert rule "+ruleN);
				if (rl[0].equals("in")) {
...
//					snt.get(r).lable=rl[3];
		...
		//			w.lable =rl[1];
	//				w.plable =rl[1];
					snt.get(r).lable=rl[1];
					snt.get(r).plable=rl[1];
		//			DB.println("relabel "+rl[1]);
					// add as new node later
	//				newNodes.add(w);				
