...
public final class Options extends OptionsSuper {
	...
	public String dsfile =null;
	public String dstest =null;
	public String shallowMapping;
	public String deepMapping;
	public String shallowMappingTest;
	public String deepMappingTest;
	public String convertFile =null;
	...
	public Options (String[] args) {
		...
		for(int i = 0; i < args.length; i++) {
			...
			if (pair[0].equals("--help")) explain();
			...
			else if (pair[0].equals("-ds")) {
				dsfile = args[i+1]; i++;
			}
			else if (pair[0].equals("-testds")) {
				dstest = args[i+1]; i++;
			}
				...
				nopred =true;	 
			} else if (pair[0].equals("-float")) {
				flt =true;	 
			} else if (pair[0].equals("-features")) {
				features= args[i+1]; i++;			
			} else if (pair[0].equals("-hsize")) {
				hsize= Integer.parseInt(args[i+1]); i++;			
			} else if (pair[0].equals("-len")) {
				maxLen= Integer.parseInt(args[i+1]); i++;			
			} else if (pair[0].equals("-cores")) {
				cores= Integer.parseInt(args[i+1]); i++;	
			} else if (pair[0].equals("-shallow")) {
				this.shallowMapping= args[++i]; 	
			} else if (pair[0].equals("-deep")) {
				this.deepMapping= args[++i]; 	
			} else if (pair[0].equals("-shallowTest")) {
				this.shallowMappingTest= args[++i]; 	
			} else if (pair[0].equals("-deepTest")) {
				this.deepMappingTest= args[++i]; 	
			} else if (pair[0].equals("-convert")) {
				this.convertFile= args[++i]; 	
			} else System.out.println("Option unkown "+pair[0]);
