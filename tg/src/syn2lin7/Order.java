...
final public class Order implements Comparable <Order> {
	...
	public boolean isGold() {
		for(int k=0;k<nodes.length();k++) if ((int)nodes.charAt(k) != k ) return false;
		return true;
	}
	...
	public boolean check(int len ) {
		...
		for(int k=0;k<len-1;k++) {
			...
			if (((int)nodes.charAt(k)) >=len) {
				new Exception().printStackTrace();
				System.out.println("((int)nodes.charAt(k)) "+((int)nodes.charAt(k))+" len "+len);
		//		System.exit(0);
				return false;
			}
		}
