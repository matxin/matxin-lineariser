...
final public class Pipe extends PipeGen {
	...
	throws Exception {
		...
		//	if (num1 % 100 == 0)  
			...
			for(int k=0;k<is.length(is.size()-1);k++) {
				...
				if (is.glemmas[is.size()-1][k]<0) emptyGLemma++;
			}
	...
	public double errors1(int[]nodes, Instances is, int i) {
		
		if (nodes.length==1) return 0;
		double errors=0;
		
		
		// build gold node list 
...
//		}
	...
	//		System.out.println(""+nodes[i]+" "+nodes[i+1]);
			...
			// a better metric
			if (is.glemmas[i][nodeList.get(j)] != is.glemmas[i][nodes[j]] || is.gpos[i][nodeList.get(j)] != is.gpos[i][nodes[j]]) { 
...
*/
		for (int j=0;j<nodes.length-1;j++) {
					...
					// a better metric
	//		int[] children = morph2.Extractor.getChildren2(j, is, i);
			//	extractor[0].getChildren
