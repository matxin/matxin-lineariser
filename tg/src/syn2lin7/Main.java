		...
		for(int i=0;i<is.size();i++) {
			...
			for(int h=0;h<is.length(i);h++) {
				...
				for(int c : children) {
					...
					int children2[] =getChildren(is,i,c);
		//			if (children2!=null && children2.length>0) continue;
					
					String signature = wds[is.glemmas[i][c]]+" "+
					px[is.gpos[i][c]]+" "+rel[is.labels[i][c]]+"\t"+
					is.gfeats[i][c]+"\t"+
					is.predicat[i][c] ; 
...
//DB.println("sign "+signature+" "+is.glemmas[i][c]+" "+wds[is.glemmas[i][c]]);
				...
				for(Entry<String,ArrayList<Integer>>  e :equalChildren.entrySet()) {
					...
					for(int j : e.getValue()) {
						is.predicat[i][j] =cnt++;
				//		System.out.println(i+" sig '"+e.getKey()+"' "+e.getValue()+" "+j+" "+wds[is.glemmas[i][j]]);
					}
				}
...
*/
		...
		for(; iter < options.numIters; iter++) {
			...
			for(int n = 0; n < numInstances; n++) {
				...
				// warning for a test the root is 0
				...
				for(int k=0;k<l.wordChilds.length;k++) {
					...
					double e = pipe.errors1(l.results[node][0].path,is,n);
...
*/
			...
			// sort the children so that we can access those recursively 
			HashSet<Integer> done = new HashSet<Integer>();
			while(true) {
...
//				if (cnt ==23) {
//					System.out.print(k+" path of node "+node+" path ");
...
//					System.out.println("   "+l.results[node][0].prob);
	//			}
			...
			for(int k=0;k<is.length(0);k++) {
				...
				if (is.heads[0][k]==0) {
					if(found ==true){
						System.out.println("!!!!!!!!!!! root already found "+root+" k "+k);
						//						break;
						//			queue.add(0);
					}
					found =true;
			...
			if (cnt ==23) {
				DB.println(""+l.toString());
			}
			
			List<Order> orders = getLeftN(l,root,50,10,cnt); // 30,9
		...
		//	List<Order> orders = getLeftN(l,root,30,6,params,pipe.extractor[0],is,0,-1); // 8 would be better
			if (cnt ==23) {
				DB.println(""+orders.get(0).toString());
			}
			

			for(int o=0;o<orders.size();o++) {
				if (! orders.get(o).check(is.length(0))) {
					DB.println(""+orders);
					System.exit(0);
				}
			}
				...
				for(int j = 0; j < instance.length()-1; j++) {
					...
					try {
						...
						for(int k=0;k<all.length;k++) {
							DB.println(" all "+k+" "+all[k]);
						}
						DB.println(cnt+" al  "+al);
						DB.println("j "+j+" all.length "+all.length);
						DB.println("is.length(0) "+is.length(0));
						System.exit(0);
					}
				}

		//		System.out.println(s.toString()+"\t"+o.p+"\t"+o.wrong());
			}
...
/*
			...
			*/
			for(int j = 0; j < formsNoRoot.length; j++) {
				...
				else heads[j] =jx.get(instance.heads[w])+1; //instance.heads[
	...
	private static int languageModel(SentenceData09 instance, List<Order> orders, NgramLanguageModel languageModel) {
		...
		for(int o=0;o<orders.size();o++) {
			...
			for(int j = 0; j < instance.length()-1; j++) {
				...
				int w= all[j]+1;
				if (s.length()>0) s.append(" ");
				s.append(instance.lemmas[w]);
				snt[j] =instance.lemmas[w];
	...
	final public static List<Order> getLeftN(Linear l, int root, int cnt,int pm ) {
		return getLeftN( l, root, cnt,pm, -1 );
	}


	final public static List<Order> getLeftN(Linear l, int root, int cnt,int pm, int num ) {

		List<Order> allOrders =new ArrayList<Order>();

		
		if(num ==23) {
			DB.println("root "+ root);
		}
